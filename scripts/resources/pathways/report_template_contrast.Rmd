```{r include=FALSE}
# Prepare contrast-specific data: extract pathway results, format hover text,
# and set up caption counters for the current contrast
  curr.con <- "{{con}}"
...
```{r echo=TRUE, fig.cap=capTab("Significantly impacted pathways and their associated accumulation (magnitude of the impact) and p-values")}
# Render an interactive DataTable of significantly impacted pathways with their
# accumulation scores, p-values, and FDR-adjusted p-values
...
```{r echo=TRUE, fig.align="center", fig.cap=capFig("**Pathways perturbation vs over-representation**: Pathways are plotted in terms of the two types of evidence computed by MITHrIL: over-representation on the x-axis (pORA) and the total pathway accumulation on the y-axis (pAcc). Each pathway is represented by a single dot. Their color represents the combined p-value. Both p-values are shown in terms of their negative log (base 10) values.")}
# Generate a scatterplot of pathway perturbation (pAcc) vs over-representation (pORA)
# Color represents combined p-value; dashed lines mark significance thresholds
